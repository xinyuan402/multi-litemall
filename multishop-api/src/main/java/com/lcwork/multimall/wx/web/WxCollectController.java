...
public class WxCollectController {
    ...
    public Object list(@LoginUser Integer userId,
        ...
        List<MultimallCollect> collectList = collectService.queryByType(userId, type, page, limit, sort, order);

        List<Object> collects = new ArrayList<>(collectList.size());
        for (MultimallCollect collect : collectList) {
            Map<String, Object> c = new HashMap<String, Object>();
            c.put("id", collect.getId());
            c.put("type", collect.getType());
            c.put("valueId", collect.getValueId());

            MultimallGoods goods = goodsService.findById(collect.getValueId());
    ...
    public Object addordelete(@LoginUser Integer userId, @RequestBody String body) {
        ...
        MultimallCollect collect = collectService.queryByTypeAndValue(userId, type, valueId);

        if (collect != null) {
            collectService.deleteById(collect.getId());
        } else {
            collect = new MultimallCollect();
