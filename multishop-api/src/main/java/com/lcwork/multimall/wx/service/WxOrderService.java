...
import com.lcwork.multimall.core.notify.NotifyService;
import com.lcwork.multimall.core.notify.NotifyType;
import com.lcwork.multimall.core.qcode.QCodeService;
import com.lcwork.multimall.core.system.SystemConfig;
import com.lcwork.multimall.core.task.TaskService;
import com.lcwork.multimall.core.util.DateTimeUtil;
import com.lcwork.multimall.core.util.IpUtil;
...
import com.lcwork.multimall.db.util.OrderHandleOption;
import com.lcwork.multimall.db.util.OrderUtil;
import com.lcwork.multimall.wx.task.OrderUnpaidTask;
...
import static com.lcwork.multimall.wx.util.WxResponseCode.*;
...
public class WxOrderService {
    ...
    @Autowired
    private MultimallCartService cartService;
    @Autowired
    ...
    @Autowired
    private MultimallAftersaleService aftersaleService;
    ...
    public Object list(Integer userId, Integer showType, Integer page, Integer limit, String sort, String order) {
        ...
        List<MultimallOrder> orderList = orderService.queryByOrderStatus(userId, orderStatus, page, limit, sort, order);

        List<Map<String, Object>> orderVoList = new ArrayList<>(orderList.size());
        for (MultimallOrder o : orderList) {
            ...
            MultimallGroupon groupon = grouponService.queryByOrderId(o.getId());
            if (groupon != null) {
            ...
            List<MultimallOrderGoods> orderGoodsList = orderGoodsService.queryByOid(o.getId());
    ...
    public Object detail(Integer userId, Integer orderId) {
        ...
        // 订单信息
        MultimallOrder order = orderService.findById(userId, orderId);
        if (null == order) {
    ...
    public Object submit(Integer userId, String body) {
        ...
        if (grouponRulesId != null && grouponRulesId > 0) {
            MultimallGrouponRules rules = grouponRulesService.findById(grouponRulesId);
            //找不到记录
            if (rules == null) {
                return ResponseUtil.badArgument();
            }
            ...
            if (grouponLinkId != null && grouponLinkId > 0) {
                ...
                // （2）不允许参加自己开团的团购
                MultimallGroupon groupon = grouponService.queryById(userId, grouponLinkId);
        ...
        // 收货地址
        MultimallAddress checkedAddress = addressService.query(userId, addressId);
        ...
        MultimallGrouponRules grouponRules = grouponRulesService.findById(grouponRulesId);
        if (grouponRules != null) {
            grouponPrice = grouponRules.getDiscount();
        }

        // 货品价格
        List<MultimallCart> checkedGoodsList = null;
        if (cartId.equals(0)) {
            checkedGoodsList = cartService.queryByUidAndChecked(userId);
        } else {
            MultimallCart cart = cartService.findById(cartId);
        ...
        for (MultimallCart checkGoods : checkedGoodsList) {
        ...
        if (couponId != 0 && couponId != -1) {
            MultimallCoupon coupon = couponVerifyService.checkCoupon(userId, couponId, userCouponId, checkedGoodsPrice);
            if (coupon == null) {
                return ResponseUtil.badArgumentValue();
            }
        ...
        MultimallOrder order = null;
        // 订单
        order = new MultimallOrder();
        ...
        // 添加订单商品表项
        for (MultimallCart cartGoods : checkedGoodsList) {
            // 订单商品
            MultimallOrderGoods orderGoods = new MultimallOrderGoods();
        ...
        for (MultimallCart checkGoods : checkedGoodsList) {
            ...
            MultimallGoodsProduct product = productService.findById(productId);
        ...
        //如果是团购项目，添加团购信息
        if (grouponRulesId != null && grouponRulesId > 0) {
            MultimallGroupon groupon = new MultimallGroupon();
            ...
            if (grouponLinkId != null && grouponLinkId > 0) {
                //参与的团购记录
                MultimallGroupon baseGroupon = grouponService.queryById(grouponLinkId);
    ...
    public Object payNotify(HttpServletRequest request, HttpServletResponse response) {
        ...
        MultimallOrder order = orderService.findBySn(orderSn);
        if (order == null) {
        ...
        //  支付成功，有团购信息，更新团购信息
        MultimallGroupon groupon = grouponService.queryByOrderId(order.getId());
        if (groupon != null) {
            MultimallGrouponRules grouponRules = grouponRulesService.findById(groupon.getRulesId());
            ...
            List<MultimallGroupon> grouponList = grouponService.queryJoinRecord(groupon.getGrouponId());
            if (groupon.getGrouponId() != 0 && (grouponList.size() >= grouponRules.getDiscountMember() - 1)) {
                for (MultimallGroupon grouponActivity : grouponList) {
                    grouponActivity.setStatus(GrouponConstant.STATUS_SUCCEED);
                    grouponService.updateById(grouponActivity);
                }

                MultimallGroupon grouponSource = grouponService.queryById(groupon.getGrouponId());
    ...
    public Object goods(Integer userId, Integer orderId, Integer goodsId) {
        ...
        List<MultimallOrderGoods> orderGoodsList = orderGoodsService.findByOidAndGid(orderId, goodsId);
        ...
        MultimallOrderGoods orderGoods = orderGoodsList.get(0);
    ...
    public Object comment(Integer userId, String body) {
        ...
        MultimallOrderGoods orderGoods = orderGoodsService.findById(orderGoodsId);
        if (orderGoods == null) {
            return ResponseUtil.badArgumentValue();
        }
        ...
        // 1. 创建评价
        MultimallComment comment = new MultimallComment();
