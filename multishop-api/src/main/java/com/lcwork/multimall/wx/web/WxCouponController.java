...
import com.lcwork.multimall.db.service.MultimallCartService;
...
import com.lcwork.multimall.wx.vo.CouponVo;
...
public class WxCouponController {
    ...
    public Object list(@RequestParam(defaultValue = "1") Integer page,
        ...
        List<MultimallCoupon> couponList = couponService.queryList(page, limit, sort, order);
    ...
    public Object mylist(@LoginUser Integer userId,
        ...
        List<MultimallCouponUser> couponUserList = couponUserService.queryList(userId, null, status, page, limit, sort, order);
        List<CouponVo> couponVoList = change(couponUserList);
        return ResponseUtil.okList(couponVoList, couponUserList);
    }

    private List<CouponVo> change(List<MultimallCouponUser> couponList) {
        List<CouponVo> couponVoList = new ArrayList<>(couponList.size());
        for (MultimallCouponUser couponUser : couponList) {
    ...
    public Object selectlist(@LoginUser Integer userId, Integer cartId, Integer grouponRulesId) {
        ...
        for (CouponVo cv : couponVoList) {
            MultimallCoupon coupon = couponVerifyService.checkCoupon(userId, cv.getCid(), cv.getId(), checkedGoodsPrice);
    ...
    public Object exchange(@LoginUser Integer userId, @RequestBody String body) {
        ...
        MultimallCoupon coupon = couponService.findByCode(code);
        if (coupon == null) {
