...
import com.lcwork.multimall.db.service.MultimallSearchHistoryService;
...
public class WxSearchController {
    ...
    @Autowired
    private MultimallKeywordService keywordsService;
    @Autowired
    ...
    public Object index(@LoginUser Integer userId) {
        //取出输入框默认的关键词
        MultimallKeyword defaultKeyword = keywordsService.queryDefault();
        //取出热闹关键词
        List<MultimallKeyword> hotKeywordList = keywordsService.queryHots();

        List<MultimallSearchHistory> historyList = null;
        if (userId != null) {
            //取出用户历史关键字
    ...
    public Object helper(@NotEmpty String keyword,
        ...
        List<MultimallKeyword> keywordsList = keywordsService.queryByKeyword(keyword, page, limit);
        String[] keys = new String[keywordsList.size()];
        int index = 0;
        for (MultimallKeyword key : keywordsList) {
