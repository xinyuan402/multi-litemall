...
@Service
public class MultimallCouponUserService {
    @Resource
    ...
    public void add(MultimallCouponUser couponUser) {
        couponUser.setAddTime(LocalDateTime.now());
        couponUser.setUpdateTime(LocalDateTime.now());
        couponUserMapper.insertSelective(couponUser);
    }

    public List<MultimallCouponUser> queryList(Integer userId, Integer couponId, Short status, Integer page, Integer size, String sort, String order) {
        MultimallCouponUserExample example = new MultimallCouponUserExample();
        MultimallCouponUserExample.Criteria criteria = example.createCriteria();
        if (userId != null) {
    ...
    public List<MultimallCouponUser> queryAll(Integer userId, Integer couponId) {
        return queryList(userId, couponId, CouponUserConstant.STATUS_USABLE, null, null, "add_time", "desc");
    }

    public List<MultimallCouponUser> queryAll(Integer userId) {
        return queryList(userId, null, CouponUserConstant.STATUS_USABLE, null, null, "add_time", "desc");
    }

    public MultimallCouponUser queryOne(Integer userId, Integer couponId) {
        List<MultimallCouponUser> couponUserList = queryList(userId, couponId, CouponUserConstant.STATUS_USABLE, 1, 1, "add_time", "desc");
    ...
    public MultimallCouponUser findById(Integer id) {
        return couponUserMapper.selectByPrimaryKey(id);
    }


    public int update(MultimallCouponUser couponUser) {
        couponUser.setUpdateTime(LocalDateTime.now());
        return couponUserMapper.updateByPrimaryKeySelective(couponUser);
    }

    public List<MultimallCouponUser> queryExpired() {
        MultimallCouponUserExample example = new MultimallCouponUserExample();
