...
import static com.lcwork.multimall.sysadmin.util.SysadminResponseCode.*;

@Service

public class SysadminOrderService {
    private final Log logger = LogFactory.getLog(SysadminOrderService.class);
    ...
    public Object list(Integer userId, String orderSn, LocalDateTime start, LocalDateTime end, List<Short> orderStatusArray,
        ...
        List<MultimallOrder> orderList = orderService.querySelective(userId, orderSn, start, end, orderStatusArray, page, limit,
                sort, order);
        return ResponseUtil.okList(orderList);
    }

    public Object detail(Integer id) {
        MultimallOrder order = orderService.findById(id);
        List<MultimallOrderGoods> orderGoods = orderGoodsService.queryByOid(id);
    ...
    public Object reply(String body) {
        ...
        // 目前只支持回复一次
        MultimallComment comment = commentService.findById(commentId);
        if (comment == null) {
            return ResponseUtil.badArgument();
        }
