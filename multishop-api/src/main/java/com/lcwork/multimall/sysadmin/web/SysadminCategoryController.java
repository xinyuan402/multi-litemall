...
import com.lcwork.multimall.db.service.MultimallCategoryService;
import com.lcwork.multimall.sysadmin.vo.CategoryVo;
...
@RestController
@RequestMapping("/sysadmin/category")
@Validated
public class SysadminCategoryController {
    private final Log logger = LogFactory.getLog(SysadminCategoryController.class);

    @Autowired
    private MultimallCategoryService categoryService;

    @RequiresPermissions("sysadmin:category:list")
    @RequiresPermissionsDesc(menu = {"商场管理", "类目管理"}, button = "查询")
    @GetMapping("/list")
    public Object list() {
        List<CategoryVo> categoryVoList = new ArrayList<>();

        List<MultimallCategory> categoryList = categoryService.queryByPid(0);
        for (MultimallCategory category : categoryList) {
            ...
            List<MultimallCategory> subCategoryList = categoryService.queryByPid(category.getId());
            for (MultimallCategory subCategory : subCategoryList) {
    ...
    private Object validate(MultimallCategory category) {
    ...
    @RequiresPermissions("sysadmin:category:create")
    @RequiresPermissionsDesc(menu = {"商场管理", "类目管理"}, button = "添加")
    @PostMapping("/create")
    public Object create(@RequestBody MultimallCategory category) {
    ...
    @RequiresPermissions("sysadmin:category:read")
    @RequiresPermissionsDesc(menu = {"商场管理", "类目管理"}, button = "详情")
    @GetMapping("/read")
    public Object read(@NotNull Integer id) {
        MultimallCategory category = categoryService.findById(id);
        return ResponseUtil.ok(category);
    }

    @RequiresPermissions("sysadmin:category:update")
    @RequiresPermissionsDesc(menu = {"商场管理", "类目管理"}, button = "编辑")
    @PostMapping("/update")
    public Object update(@RequestBody MultimallCategory category) {
        Object error = validate(category);
        if (error != null) {
            return error;
        }
    ...
    @RequiresPermissions("sysadmin:category:delete")
    @RequiresPermissionsDesc(menu = {"商场管理", "类目管理"}, button = "删除")
    @PostMapping("/delete")
    public Object delete(@RequestBody MultimallCategory category) {
    ...
    public Object catL1() {
        // 所有一级分类目录
        List<MultimallCategory> l1CatList = categoryService.queryL1();
        List<Map<String, Object>> data = new ArrayList<>(l1CatList.size());
        for (MultimallCategory category : l1CatList) {
