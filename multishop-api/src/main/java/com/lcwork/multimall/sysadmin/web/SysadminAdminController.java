...
import com.lcwork.multimall.core.util.bcrypt.BCryptPasswordEncoder;
...
import com.lcwork.multimall.sysadmin.service.SysadminLogHelper;
...
public class SysadminAdminController {
    private final Log logger = LogFactory.getLog(SysadminAdminController.class);
    ...
    @RequiresPermissions("sysadmin:admin:list")
    ...
    public Object list(String username,
        ...
        List<MultimallAdmin> adminList = adminService.querySelective(username, page, limit, sort, order);
        return ResponseUtil.okList(adminList);
    }

    private Object validate(MultimallAdmin admin) {
    ...
    @RequiresPermissions("sysadmin:admin:create")
    @RequiresPermissionsDesc(menu = {"系统管理", "管理员管理"}, button = "添加")
    @PostMapping("/create")
    public Object create(@RequestBody MultimallAdmin admin) {
        ...
        List<MultimallAdmin> adminList = adminService.findAdmin(username);
        if (adminList.size() > 0) {
    ...
    @RequiresPermissions("sysadmin:admin:read")
    @RequiresPermissionsDesc(menu = {"系统管理", "管理员管理"}, button = "详情")
    @GetMapping("/read")
    public Object read(@NotNull Integer id) {
        MultimallAdmin admin = adminService.findById(id);
        return ResponseUtil.ok(admin);
    }

    @RequiresPermissions("sysadmin:admin:update")
    @RequiresPermissionsDesc(menu = {"系统管理", "管理员管理"}, button = "编辑")
    @PostMapping("/update")
    public Object update(@RequestBody MultimallAdmin admin) {
        Object error = validate(admin);
        if (error != null) {
            return error;
        }
    ...
    @RequiresPermissions("sysadmin:admin:delete")
    @RequiresPermissionsDesc(menu = {"系统管理", "管理员管理"}, button = "删除")
    @PostMapping("/delete")
    public Object delete(@RequestBody MultimallAdmin admin) {
        Integer anotherAdminId = admin.getId();
        if (anotherAdminId == null) {
            return ResponseUtil.badArgument();
        }
        ...
        MultimallAdmin currentAdmin = (MultimallAdmin) currentUser.getPrincipal();
