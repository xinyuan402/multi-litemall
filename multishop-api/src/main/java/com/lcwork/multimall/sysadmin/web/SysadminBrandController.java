...
@RestController
@RequestMapping("/sysadmin/brand")
@Validated
public class SysadminBrandController {
    private final Log logger = LogFactory.getLog(SysadminBrandController.class);

    @Autowired
    private MultimallBrandService brandService;

    @RequiresPermissions("sysadmin:brand:list")
    ...
    public Object list(String id, String name,
        ...
        List<MultimallBrand> brandList = brandService.querySelective(id, name, page, limit, sort, order);
        return ResponseUtil.okList(brandList);
    }

    private Object validate(MultimallBrand brand) {
    ...
    @RequiresPermissions("sysadmin:brand:create")
    @RequiresPermissionsDesc(menu = {"商场管理", "品牌管理"}, button = "添加")
    @PostMapping("/create")
    public Object create(@RequestBody MultimallBrand brand) {
    ...
    @RequiresPermissions("sysadmin:brand:read")
    @RequiresPermissionsDesc(menu = {"商场管理", "品牌管理"}, button = "详情")
    @GetMapping("/read")
    public Object read(@NotNull Integer id) {
        MultimallBrand brand = brandService.findById(id);
        return ResponseUtil.ok(brand);
    }

    @RequiresPermissions("sysadmin:brand:update")
    @RequiresPermissionsDesc(menu = {"商场管理", "品牌管理"}, button = "编辑")
    @PostMapping("/update")
    public Object update(@RequestBody MultimallBrand brand) {
        Object error = validate(brand);
        if (error != null) {
            return error;
        }
    ...
    @RequiresPermissions("sysadmin:brand:delete")
    @RequiresPermissionsDesc(menu = {"商场管理", "品牌管理"}, button = "删除")
    @PostMapping("/delete")
    public Object delete(@RequestBody MultimallBrand brand) {
